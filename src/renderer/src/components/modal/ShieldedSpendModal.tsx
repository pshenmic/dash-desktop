...
function HashField({ hash }: { hash: string }): React.JSX.Element {
  ...
  return (
    <div className={"flex flex-col gap-[.375rem]"}>
      ...
      <button onClick={copy} title={"Click to copy"} className={"text-left px-3 py-2.5 rounded-[.75rem] dash-block-accent-5 dash-black-border cursor-pointer hover:dash-block-accent-10 transition-colors duration-200"}>
        <Text size={12} weight={"medium"} color={"brand"} className={"font-mono break-all select-all leading-[140%]"}>{hash}</Text>
      </button>
