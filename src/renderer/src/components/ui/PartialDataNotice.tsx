...
export default function PartialDataNotice({ className = '' }: { className?: string }): React.JSX.Element | null {
  const { dataIncomplete } = useConnectionModeContext()

  if (!dataIncomplete) return null

  return (
    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-[.625rem] dash-block-3 self-start ${className}`}>
