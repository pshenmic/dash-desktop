  ...
  return (
    <div className={'flex flex-col gap-4 px-12 pb-8'}>
      ...
      <div className={'flex flex-col gap-5 p-[.9375rem] rounded-[.9375rem] dash-card-base shadow-[0_0_50px_0_rgba(0,0,0,0.1)]'}>
        ...
        {transaction.sender.map((sender, index) => (
          <HashField
            key={`sender-${index}`}
            label={'From'}
            hash={sender.source}
            explorerUrl={network ? platformParticipantUrl(sender.source, network) : undefined}
          />
        ))}
        {transaction.recipient.map((recipient, index) => (
          <HashField
            key={`recipient-${index}`}
            label={'To'}
            hash={recipient.source}
            explorerUrl={network ? platformParticipantUrl(recipient.source, network) : undefined}
          />
        ))}
      </div>
    </div>
