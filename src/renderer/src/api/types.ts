...
// What an entry is, so it can be labelled without re-deriving the operation.
...
export interface PreviewEntry {
  role: PreviewRole
  // Empty where the operation creates what it pays: a new identity, or a
  // shielded change note whose diversified address the worker derives.
  address: string
  amount: bigint
  unit: PreviewUnit
...
// would pay, and the price — read off the selection the send itself runs, not
// estimated a second time here.
...
// duffs wherever L1 funds the transaction, credits everywhere else.
...
export interface PlatformTransaction {
  ...
  amountCredits: bigint
  // Addresses or identities; empty when the source did not name that side.
  sender: string[]
  recipient: string[]
}
