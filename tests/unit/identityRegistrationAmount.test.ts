...
describe('identityRegistrationAmountError', () => {
  it('accepts the 0.1 Dash minimum when the balance covers the Core fee', () => {
    expect(identityRegistrationAmountError('0.1', 10_000_000n, 10_010_000n, FEE)).toBeNull()
  })

  it('rejects an amount below the identity funding minimum', () => {
    expect(identityRegistrationAmountError('0.09999999', 9_999_999n, 100_000_000n, FEE))
  ...
  it('rejects malformed and over-precision amounts', () => {
    expect(identityRegistrationAmountError('1.2.3', 0n, 100_000_000n, FEE))
      .toBe('Enter a valid Dash amount with up to 8 decimal places.')
    expect(identityRegistrationAmountError('0.123456789', 12_345_678n, 100_000_000n, FEE))
      .toBe('Enter a valid Dash amount with up to 8 decimal places.')
  })

  it('keeps an empty field neutral while the UI disables advancement', () => {
    expect(identityRegistrationAmountError('', 0n, 100_000_000n, FEE)).toBeNull()
  })
})
