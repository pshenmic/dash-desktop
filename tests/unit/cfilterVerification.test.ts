...
// A wallet with no persisted cursor starts at height 1, and height 1 chains off
// a real block below it — the peer supplies that base as `previousFilterHeader`,
// it is not the zero hash. Treating it as zero made every filter at height 1
// fail verification, which wedged the batch and held the scan in `cfilters`
// forever. A resumed wallet starts at cursor+1 and never reaches height 1, so
// this only shows on a fresh one.
describe('cfilter verification at the bottom of the chain', () => {
  let pool: FakePool
  let worker: CFilterSyncWorker
  ...
  const BASE = new Uint8Array(32).fill(0xab)

  const seedFromBase = (through: number, filter: Uint8Array): void => {
    const index = filterHeaders()
    index.set(0, BASE)
    let prev: Uint8Array = BASE
    for (let h = 1; h <= through; h++) {
      prev = deriveFilterHeader(hashFilter(filter), prev)
      index.set(h, prev)
    }
  }
  ...
  it('accepts the filter at height 1 against the recorded base', () => {
    seedFromBase(TIP + 4, EMPTY_FILTER)
    worker.onChainExtended([header(TIP + 1), header(TIP + 2)])

    const batch = [...batches().values()][0]
    expect(batch).toBeDefined()
    expect(batch!.remaining.has(1)).toBe(true)

    pool.emit('peercfilter', pool.peer, {
      blockHash: displayHexToWire(GENESIS.testnet.hash),
      filter: EMPTY_FILTER,
    })

    expect(batch!.remaining.has(1)).toBe(false)
  })

  it('holds the scan while the base is unknown rather than scanning unverified', () => {
    // Same chain, but nothing recorded below height 1.
    const index = filterHeaders()
    let prev: Uint8Array = BASE
    for (let h = 1; h <= TIP + 4; h++) {
      prev = deriveFilterHeader(hashFilter(EMPTY_FILTER), prev)
      index.set(h, prev)
    }
